...
class SklearnSGDFLModel(SklearnFLModel):
    ...
    def __init__(self, model_name, model_spec, sklearn_model=None, **kwargs):
        ...
        if sklearn_model:
            ...
            if type(sklearn_model) is SGDClassifier:
                self.is_classification = True
            elif type(sklearn_model) is SGDRegressor:
    ...
    def fit_model(self, train_data, fit_params=None):
        ...
        try:
            if hyperparams is not None and 'local' in hyperparams and \
                    hyperparams['local'] is not None and \
                    'training' in hyperparams['local']:
    ...
    def save_model(self, filename=None, path=None):
        ...
        """
        if filename is None:
            filename = self.model_name + '_{}.pickle'.format(time.time())
