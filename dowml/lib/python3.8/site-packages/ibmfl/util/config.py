...
def configure_logging_from_file(config_file='log_config.yaml',
    ...
    result = log_config.configure_logging(config, log_level=logging.INFO)
...
def get_fusion_from_config(config):
    ...
    if config:
        ...
        logger.info('No fusion config provided for this setup.')
...
def get_lt_from_config(config):
    ...
    if config:
        ...
        logger.info('No local training config provided for this setup.')
...
def get_class_by_name(path, name_class):
    ...
    try:
        ...
        logger.exception(ex)
        raise InvalidConfigurationException(
            'Error occurred while loading class %s', name_class,
            'from path %s', path)
    return cls_ref
...
def get_attr_from_path(path, attr):
    ...
    if is_file_path(path):
        ...
        if absolute_path:
            ...
            raise InvalidConfigurationException(
                "File does not exists! " + "file with given path "
                + path+" not found in both current and working directory!!")
...
def get_absolute_path(path):
    ...
    if file_path.is_absolute == True and not file_path.exists():
        raise InvalidConfigurationException(
            "File does not exists! " + "file with given path " + path+" not found!!")
    ...
    if absolute_path is None or os.path.exists(absolute_path) is False:
        raise InvalidConfigurationException(
            "File does not exists! " + "could not find file !" + path + 'in both current and working directory')
