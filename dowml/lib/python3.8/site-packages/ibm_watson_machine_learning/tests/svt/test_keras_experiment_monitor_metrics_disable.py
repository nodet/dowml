...
class TestWMLClientWithKeras(unittest.TestCase):
    ...
    def test_09_monitor_metrics(self):
        queue = Queue()
        process = Process(target=run_monitor_metrics, args=(self.client, TestWMLClientWithKeras.experiment_run_uid, queue))
        process.start()
        process.join(timeout=90)
        self.assertFalse(queue.empty())
        self.assertTrue("loss" in queue.get())

    def test_10_get_status(self):
        start_time = time.time()
        diff_time = start_time - start_time
        while True and diff_time < 60 * 15:
            time.sleep(3)
        ...
        TestWMLClientWithKeras.logger.info("Experiment status: {}".format(status))
    ...
    #     TestWMLClientWithCaffe.trained_model_uid = self.client.repository.store_model(TestWMLClientWithCaffe.train_run_uid, "test keras")
