...
class TestAutoAIRemote(unittest.TestCase):
    ...
    bucket_name = "pycharm"
    data_cos_path = 'data/insurance.csv'
    ...
    def setUp(cls) -> None:
        ...
        cls.X = cls.data.drop(['charges'], axis=1)
        cls.y = cls.data['charges']
    ...
    def test_03_initialize_optimizer(self):
        TestAutoAIRemote.remote_auto_pipelines = self.experiment.optimizer(
            ...
            daub_include_only_estimators=[self.experiment.RegressionAlgorithms.XGB]
        )
    ...
    def test_08_predict_using_fitted_pipeline(self):
        predictions = self.remote_auto_pipelines.predict(X=self.holdout_data.drop(['charges'], axis=1).values[:5])
        print(predictions)
    ...
    def test_12_get_pipeline_load_specified_pipeline(self):
        ...
        #     X=self.holdout_data.drop(['price'], axis=1).values[:5])  # works with autoai_libs from branch `development` (date: 9th March 20)
        # print(predictions)
